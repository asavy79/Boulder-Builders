...
export default function AddSkill() {
    ...
    useEffect(() => {
        const fetchSkills = async () => {
            ...
            setSkills(data.map((skill: any) => ({name: skill.skill.name, id: skill.skill_id})));
        }
    ...
    const handleRemoveSkill = async (skillToRemove: {name: string, id: string}) => {
        const response = await fetch("/api/users/skills", {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-6 space-y-6">
            <div className="space-y-2">
                <h2 className="text-2xl font-semibold text-gray-800">Add Your Skills</h2>
